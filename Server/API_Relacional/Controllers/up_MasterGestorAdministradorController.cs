...
namespace API_Relacional.Controllers
{
    ...
    [ApiController]
    public class up_MasterGestorAdministradorController : ControllerBase
    {
        ...
        public up_MasterGestorAdministradorController(IConfiguration configuration)
        {
        ...
        [HttpPost]
        public JsonResult Post(Administrador x, string statementtype)
        {
            string query = @"
                exec dbo.up_MasterGestorAdministrador @adminid, @correo, @password, @statementtype
                ";
            ...
            using (SqlConnection connection = new SqlConnection(sqlDataSource))//Se utiliza la fuente de datos para hacer la conexion
            {
                ...
                using (SqlCommand cmd = new SqlCommand(query, connection))//El comando a ejecutar se hace con un query y la conexion
                {
                    ...
                    cmd.Parameters.Add("@correo", SqlDbType.Int);
