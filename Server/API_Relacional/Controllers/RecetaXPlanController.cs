...
namespace API_Relacional.Controllers
{
    ...
    [ApiController]
    public class RecetaXPlanController : ControllerBase
    {
        ...
        public RecetaXPlanController(IConfiguration configuration)
        {
        ...
        [HttpPost]
        public JsonResult Post(RecetaXPlan x)
        {
            string query = @"
                insert into recetaxplan(idreceta, idplan)
                values (@idreceta, @idplan)";
            ...
            using (SqlConnection connection = new SqlConnection(sqlDataSource))//Se utiliza la fuente de datos para hacer la conexion
            {
                ...
                using (SqlCommand cmd = new SqlCommand(query, connection))//El comando a ejecutar se hace con un query y la conexion
                {
                    ...
                    cmd.Parameters.Add("@idplan", SqlDbType.NVarChar);
        ...
        [HttpPut("{id}")]
        public JsonResult Put(RecetaXPlan x)
        {
            string query = @"
                UPDATE recetaxplan
