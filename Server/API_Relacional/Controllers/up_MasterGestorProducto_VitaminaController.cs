...
namespace API_Relacional.Controllers
{
    ...
    [ApiController]
    public class up_MasterGestorProducto_VitaminaController : ControllerBase
    {
        ...
        public up_MasterGestorProducto_VitaminaController(IConfiguration configuration)
        {
        ...
        [HttpPost]
        public JsonResult Post(int codigodbarras, string vitaminas, string statementtype)
        {
            string query = @"
                exec dbo.up_MasterGestorProducto_Vitamina @codigodbarras, @vitaminas, @statementtype
                ";
            ...
            using (SqlConnection connection = new SqlConnection(sqlDataSource))//Se utiliza la fuente de datos para hacer la conexion
            {
                ...
                using (SqlCommand cmd = new SqlCommand(query, connection))//El comando a ejecutar se hace con un query y la conexion
                {
                    ...
                    cmd.Parameters["@codigodbarras"].Value = codigodbarras;

                    cmd.Parameters.Add("@vitaminas", SqlDbType.NVarChar);
                    cmd.Parameters["@vitaminas"].Value = vitaminas;
